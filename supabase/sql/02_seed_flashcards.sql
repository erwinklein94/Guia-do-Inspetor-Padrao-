-- =========================================================
-- GUIA DO INSPETOR PADRÃO - CARGA INICIAL DE CONTEÚDO
-- Origem: flash-cards/dados.js
-- Áreas: 5
-- Flashcards: 440
-- Questões: 50
-- Rode depois do arquivo 01_schema_auth_rls.sql.
-- =========================================================
    ...
    "quiz": [
      {
        ...
        "options": [
          "Trena, fissurômetro ou lupa para medir fissuras, e paquímetro",
          "Prumo, nível óptico, termômetro e martelete",
          "Tacômetro, goniômetro, dinamômetro e nível laser",
          "Apenas inspeção visual, sem instrumento"
        ],
        "answer": 0,
      ...
      },
      {
        ...
        "options": [
          "Aceito sem ação",
          "Reparado",
          "Liberado apenas com pintura",
          "Refugado sempre, sem avaliar dimensão"
        ],
        "answer": 1,
      ...
      },
      {
        ...
        "options": [
          "Aceitos quando estiverem fora da região do trilho",
          "Reparados com qualquer argamassa disponível",
          "Rejeitados/refugados, inclusive em peças reparadas",
          "Aprovados se a superfície for lixada"
        ],
        "answer": 2,
      ...
      },
      {
        ...
        "options": [
          "Reparar e liberar",
          "Refugar o dormente",
          "Cortar apenas a área quebrada",
          "Aguardar 24 horas e reinspecionar"
        ],
        "answer": 1,
      ...
      },
      {
        ...
        "options": [
          ...
          "Reparo superficial simples",
          "Refugo, sem permissão de reparo",
          "Apenas C2 automático"
        ],
        "answer": 2,
      ...
      },
      {
        ...
        "options": [
          "±1 mm vertical e ±1 mm horizontal",
          "±3 mm vertical e ±6 mm horizontal",
          "±6 mm vertical e ±3 mm horizontal",
          "±10 mm vertical e ±10 mm horizontal"
        ],
        "answer": 1,
      ...
      },
      {
        ...
        "options": [
          "Pintar a USP, movimentar o dormente e liberar",
          "Remover a USP danificada, lixar/preparar a superfície, restaurar quando aplicável, aplicar agente de aderência, instalar nova USP e inspecionar após secagem",
          "Cortar o dormente, aplicar óleo e colar a mesma USP",
          "Somente fotografar e arquivar"
        ],
        "answer": 1,
...
$training_json$::jsonb as data
), area_rows as (
  select
    area->>'id' as id,
    area->>'area' as nome,
    area->>'shortName' as nome_curto,
    area->>'document' as documento,
    area->>'sourceLabel' as source_label,
    ordinality::integer as ordem
  from payload, jsonb_array_elements(data) with ordinality as t(area, ordinality)
), flashcard_rows as (
  select
    area->>'id' as area_id,
    card->>'category' as categoria,
    card->>'front' as frente,
    card->>'back' as verso,
    card_ord::integer as ordem
  from payload,
       jsonb_array_elements(data) as area,
       jsonb_array_elements(area->'flashcards') with ordinality as c(card, card_ord)
), quiz_rows as (
  select
    area->>'id' as area_id,
    q->>'question' as pergunta,
    q->'options' as opcoes,
    (q->>'answer')::integer as resposta_correta,
    q->>'explanation' as explicacao,
    q_ord::integer as ordem
  from payload,
       jsonb_array_elements(data) as area,
       jsonb_array_elements(area->'quiz') with ordinality as qz(q, q_ord)
)
insert into public.areas (id, nome, nome_curto, documento, source_label, ordem, ativo)
select id, nome, nome_curto, documento, source_label, ordem, true
from area_rows
...
insert into public.flashcards (area_id, categoria, frente, verso, ordem, ativo)
select area_id, categoria, frente, verso, ordem, true
from flashcard_rows
on conflict (area_id, ordem) do update set
...
insert into public.quiz_questions (area_id, pergunta, opcoes, resposta_correta, explicacao, ordem, ativo)
select area_id, pergunta, opcoes, resposta_correta, explicacao, ordem, true
from quiz_rows
on conflict (area_id, ordem) do update set
